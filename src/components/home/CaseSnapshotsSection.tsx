      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {caseSnapshots.map((item) => (
            <article
              key={item.title}
              className="glass-panel group relative overflow-hidden rounded-2xl p-6 transition duration-300 hover:-translate-y-1"
            >
              ...
              <p className="relative terminal-kicker">
                {item.sector}
              </p>
              <h3 className="relative mt-3 font-display text-xl font-semibold text-[color:var(--color-ink)]">
                {item.title}
              </h3>
              <p className="relative mt-4 text-sm leading-relaxed text-[color:var(--color-ink-muted)]">
                <span className="font-semibold text-[color:var(--color-accent)]">Challenge:</span> {item.challenge}
              </p>
              <p className="relative mt-3 text-sm leading-relaxed text-[color:var(--color-ink-muted)]">
                <span className="font-semibold text-[color:var(--color-accent)]">Outcome:</span> {item.outcome}
              </p>
              <p className="relative mt-5 rounded-md border border-[color:var(--color-border)] bg-[color:var(--color-surface-soft)] px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--color-brand)]">
                {item.metric}
              </p>
            </article>
          ))}
